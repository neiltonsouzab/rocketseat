...
const FileProvider: React.FC = ({ children }) => {
   ...
   const uptadeFile = useCallback((id, data) => {
   ...
   const processUpload = useCallback((uploadedFile: IFile) => {
     ...
     api.post('/posts', data, {
       onUploadProgress: (progressEvent) => {
         ...
         uptadeFile(uploadedFile.id, { progress });
       },
     }).then(response => {
      console.log(`A imagem ${uploadedFile.name} foi enviada para o servidor!`);


      uptadeFile(uploadedFile.id, {
        uploaded: true,
        id: response.data._id,
        url: response.data.url,
      });
     }).catch(err => {
      console.log(`Houve um problema para fazer upload da imagem ${uploadedFile.name} no servidor AWS`);
